...
do
    ...
    do
	wget --load-cookies cookies.txt http://member.meijinsen.jp/pay/game_list/meijinsen/${year}/${class}/calendar.html -O tmp/${year}_${class}.html
    done
    if [ ! "$?" -eq 0 ]; then
	break
    fi
...
for class in M7 A1 B1 B2 C1 C2; do
    wget --load-cookies cookies.txt http://member.meijinsen.jp/pay/game_list/meijinsen/latest/${class}/calendar.html -O tmp/latest_${class}.html
done
...
do
    wget -q --load-cookies cookies.txt http://member.meijinsen.jp/${file} -O tmp/tmp;
    filehead=`echo ${file} | sed "s/[0-9]*\.html//g"`
    for idx in `grep -o -E "[0-9]+.(txt|html)" tmp/tmp | cut -d '.' -f 1,1 | sort -u`;
    do
	wget --load-cookies cookies.txt http://member.meijinsen.jp${filehead}${idx}.txt -P kif/
    done
done
